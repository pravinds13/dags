...
def my_custom_function(ts,**kwargs):
    """
    This can be python code you want and is called from the python operator. The code is not executed until
    the task is run by the airflow scheduler.
    """
    print(f"Hello! I am task number {kwargs['task_number']}. This DAG Run execution date is {ts} and the current time is {datetime.now()}")
    ...
    for task in range(5):
        if version.startswith('2'):
            tn = PythonOperator(
                ...
                python_callable=my_custom_function,  # makef sure you don't include the () of the function
